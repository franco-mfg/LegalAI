# -*- coding: utf-8 -*-
"""eurlex_Ottobre_chroma.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/1-RfxUfWp5edLrsJ1Ilti1ZSKNn88HOA1
...
EURLEX_TABLE  =os.environ.get('EURLEX_TABLE', 'test')
EURLEX_NUM_REC=os.environ.get('EURLEX_NUM_REC', '500')
...
from   eurlex.get_eurlex_id import get_lex_id
import tools.sqlitedb       as sql
...
# #########################
def split_record(record, num_doc):
...
persist_dir='./db/'+emfunc.model_name.replace('/','-')
...
for record in df_test.itertuples():
  ...
  if num_doc is not None:
    docs=split_record(record, num_doc)
    print(len(docs),'splits ...',end='\r')
    chromadb_client.add_documents(documents=docs)
  else:
    print("ERRRORE!!!")

  if(i<int(EURLEX_NUM_REC)):
...
print('\ntempo totale:',
      time.strftime('%H:%M:%S',time.gmtime(time.perf_counter()-ttime)) )
